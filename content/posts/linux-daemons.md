...
## What is a daemon. {#what-is-a-daemon-dot}
...
### Perform a fork() and terminate the parent process {#perform-a-fork-and-terminate-the-parent-process}

This allows the daemon to continue running independently of the process that started it.


### Create a new session with setsid() {#create-a-new-session-with-setsid}
...
### Change the working directory (normally to /) {#change-the-working-directory--normally-to}

This prevents the daemon from locking directories that might later need to be unmounted.


### Reset the file creation mask with umask(0) {#reset-the-file-creation-mask-with-umask--0}
...
### Use a logging system instead of printing to the terminal {#use-a-logging-system-instead-of-printing-to-the-terminal}

Normally using syslog.


### Maintain a continuous execution loop {#maintain-a-continuous-execution-loop}
...
[Create a daemon in Linux with Bash and C](https://www.youtube.com/watch?v=6uir0CCmckw)
